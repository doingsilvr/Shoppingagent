...
def build_summary_from_memory(name, mems):
    """
    메모리 리스트를 '나열'하지 않고,
    2~3문장 자연어 요약으로 정리한 후 메시지 형태로 반환하는 함수.
    """
    ...
    # ----------------------------
    # 2) GPT에게 자연스러운 요약 생성 요청
    # ----------------------------
    prompt = f"""
아래는 사용자가 알려준 '블루투스 헤드셋 쇼핑 기준' 메모리 목록입니다:

{memory_text}

이 기준들을 다음 조건에 맞춰 '2~3문장 자연어 요약'으로 정리하세요:

- 불릿 포인트 금지
- 메모리 기반으로만 요약 및 메모리 업데이트 될 때마다 동적으로 이 역시 업데이트한다.
- 메모리를 그대로 복사하지 말고 의미 단위로 자연스럽게 묶어서 표현
- 문장은 부드럽게 연결
- 너무 기계적이거나 연구 보고서 느낌이 나지 않게 작성
- 분량: 2~3문장 (약 70~120자)

출력 예시:
"전체적으로는 디자인과 스타일을 우선적으로 고려하시면서, 심플한 톤과 트렌디한 요소를 선호하시는 것 같아요. 또한 음악 감상과 휴대성 등 사용 환경도 함께 중요하게 보고 계신 것으로 이해했어요."
"""
...
제가 파악한 {name}님의 현재 쇼핑 기준을 정리해보면,
{summary_sentence}

특히 **{primary}** 부분을 가장 중요하게 고려하고 계신 것으로 보여요.
...
def handle_input():
    ...
    if not ss.priority_followup_done:
        ...
        if primary == "design":
            ai_say(
                "디자인/스타일을 중요하게 보신다고 하셔서 여쭤볼게요! "
                "전체적으로 어떤 느낌을 좋아하시나요? 미니멀, 레트로, 심플, 포인트 컬러 등 편하게 말씀해주세요 :)"
            )
        ...
        if primary == "performance":
            ai_say(
                "성능을 중요하게 보신다고 하셔서 안내드릴게요!\n"
                "보통 음질, 노이즈캔슬링, 배터리, 착용감 네 가지 요소를 많이 고려해요.\n"
                "이 중 특히 더 중요하게 생각하시는 부분이 있으실까요?"
            )
            ss.priority_followup_done = True
            return
    ...
    if ss.stage == "summary":
        if any(k in u for k in ["좋아요", "네", "추천", "맞아요"]):
            ...
            ai_say(
                "수정하고 싶은 부분이 있으시면 왼쪽 '쇼핑 메모리'에서 직접 수정할 수 있어요.\n"
