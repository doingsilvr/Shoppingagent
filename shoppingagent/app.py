...
::-webkit-scrollbar-thumb:hover {
    background: linear-gradient(180deg, #5568d3 0%, #6a3e8c 100%);
}
...
- 메모리에 실제 저장될 경우, "이 기준을 기억해둘게요", "이번 쇼핑에서는 해당 내용을 고려하지 않을게요"라고 먼저 언급한다.
...
def is_negative_response(text: str) -> bool:
    if not text:
        return False
    negative_keywords = [
        "없어", "없다고", "몰라", "모르겠", "잘 모르", "글쎄", "별로",
        "아닌데", "굳이", "그만", "필요없", "상관없", "안중요", "관심없",
    ]
