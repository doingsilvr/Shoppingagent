...
def memory_sentences_from_user_text(utter: str):
    ...
    # 1) 예산
    m = re.search(r"(\d+)\s*만\s*원", u) # 🌟 수정됨: 정규 표현식 문자열이 올바르게 닫힘
    if m:
        price = m.group(1)
...
def chat_interface():
    ...
    # 요약 단계 진입 시 요약 + 버튼
    if st.session_state.stage == "summary":
        # 1. 아직 요약이 메시지 리스트에 없다면 생성하고 리스트에 추가 (첫 번째 출력)
        summary_message_exists = any("메모리 요약" in m["content"] for m in st.session_state.messages if m["role"]=="assistant")
        
        if not summary_message_exists:
            summary_step()
            # 메시지가 추가되었으니 렌더링을 위해 rerun
            st.rerun() 
        
        # 2. 메시지 리스트에 요약이 있다면, 버튼을 그 아래에 출력 (중복 출력 방지)
