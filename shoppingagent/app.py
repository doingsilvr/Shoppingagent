...
def add_memory(mem_text: str, announce: bool = True):
    """
    메모리 추가 로직 (중복, 색상/예산 단일 유지, 최우선 기준 처리 포함)
    """
    ss = st.session_state
    ...
    mem_core = mem_text.replace("(가장 중요)", "").strip()

    # 2) 예산 기준은 항상 하나만 유지
    if "예산은 약" in mem_core:
        ss.memory = [m for m in ss.memory if "예산은 약" not in m]

    # 3) 색상 기준도 하나만 유지
    if _is_color_memory(mem_core):
        ss.memory = [m for m in ss.memory if not _is_color_memory(m)]

    # 4) 기존과 유사한 기준이 있는지 확인
    for i, m in enumerate(ss.memory):
        base = m.replace("(가장 중요)", "").strip()

        # 포함 관계(거의 같은 기준)라면
        if mem_core in base or base in mem_core:
            # (가장 중요) 승격 케이스
            if "(가장 중요)" in mem_text and "(가장 중요)" not in m:
                ...
                if announce:
                    ss.notification_message = "🌟 최우선 기준으로 설정되었어요."
                _after_memory_change()
            # 중요도 승격이 아니면 그냥 기존 메모리 유지
            return

    # 5) 완전히 새로운 기준이면 리스트에 추가
    ss.memory.append(mem_text)
    if announce:
        ss.notification_message = "🧩 메모리에 새로운 기준이 추가되었어요."
...
def delete_memory(idx: int):
    ...
    if 0 <= idx < len(ss.memory):
        del ss.memory[idx]
        ss.notification_message = "🧹 메모리에서 해당 기준을 삭제했어요."
        _after_memory_change()
...
def update_memory(idx: int, new_text: str):
    ...
    new_text = naturalize_memory(new_text or "").strip()
    if not new_text:
        return
    ...
    ss.notification_message = "🔄 메모리가 수정되었어요."
    _after_memory_change()
