...
def ss_init():
    ss = st.session_state
    ss.setdefault("page", "context_setting")          # context_setting / chat
    ss.setdefault("nickname", "")
    ss.setdefault("phone_number", "")
    ss.setdefault("condition", "A")                   # 실험 조건 A/B 등 필요시 사용
    ...
    # 단계(stage)
    ss.setdefault("stage", "explore")                 # explore → comparison → product_detail → purchase_decision
...
<style>
    ...
    .tip-box {
        ...
        padding: 16px; font-size: 14px; color: #92400E; line-height: 1.5; margin-top: 10px;
    }
...
def extract_budget(mems):
    for m in mems:
        if re.search(r"\d+만\s*원|\d{3,}원", m):
            return True
    return False

def ai_say(msg: str):
    st.session_state.messages.append({"role": "assistant", "content": msg})
    log_event("assistant_msg", {"content": msg})

def user_say(msg: str):
    st.session_state.messages.append({"role": "user", "content": msg})
    log_event("user_msg", {"content": msg})
...
def filter_products(mems, is_reroll=False):
    # TODO: 메모리 기반 필터링 로직을 여기에 확장 (지금은 TOP3 고정)
    return CATALOG[:3]
...
def generate_personalized_reason(product, mems, name):
    reasons = []
    mem_str = " ".join(mems)
    tags_joined = " ".join(product.get("tags", []))

    if "음질" in mem_str and "음질" in tags_joined:
        reasons.append("중요하게 생각하신 **음질**이 뛰어난 제품이에요.")
    if "착용감" in mem_str and "착용감" in tags_joined:
        reasons.append("오래 써도 편안한 **착용감**이 장점이에요.")
    if "디자인" in mem_str and "디자인" in tags_joined:
        reasons.append("선호하시는 **디자인** 요소를 갖추고 있어요.")
    if "가성비" in mem_str and "가성비" in tags_joined:
...
def extract_memory_with_gpt(user_input, memory_list):
    # 물어보는 질문(추천/어때/뭐야 등)은 메모리 추출 제외
    ...
    위 발화에서 쇼핑 기준(가격, 색상, 기능, 용도 등)을 한국어 문장으로 요약해 추출하세요.
    형식: {{ "memories": ["~를 선호해요", "..."] }}
    """
...
def add_memory(text, announce=True):
    ...
    if announce:
        st.session_state.notification_message = "📝 메모리에 기준이 추가되었어요!"
    log_event("memory_add", {"text": text})

def delete_memory(idx):
    if 0 <= idx < len(st.session_state.memory):
        removed = st.session_state.memory[idx]
        del st.session_state.memory[idx]
        st.session_state.memory_changed = True
        st.session_state.notification_message = "🗑️ 기준이 삭제되었습니다."
        log_event("memory_delete", {"text": removed})
...
def gpt_reply(user_input):
    ...
    # 상품 상세 단계 전용 프롬프트
    if stage == "product_detail" and st.session_state.selected_product:
        ...
        3. "현재 선택된 제품은~" 같은 메타 표현을 적절히 사용하세요.
        4. 답변 마지막에 '추가로 궁금한 점이 있다면 말씀해 주세요.' 한 문장을 자연스럽게 붙이세요.
        """
    else:
        system_prompt = f"""
        당신은 헤드셋을 도와주는 AI 쇼핑 에이전트입니다.

        [기억된 기준]
        {memories}

        [역할]
        - 사용자의 발화를 바탕으로 쇼핑 기준을 자연스럽게 파악하고 대화를 이어갑니다.
        - 기준이 이미 메모리에 있다면, 같은 내용을 반복해서 묻지 말고 "이미 ~를 중요하게 보고 계셔서"처럼 언급만 하세요.
        - 예산 관련 정보가 전혀 없으면, 너무 부담스럽지 않게 한 번 정도 예산 범위를 물어볼 수 있습니다.
        - "추천"을 언급하면, 기준이 어느 정도 모였다고 간주하고 추천 단계로 자연스럽게 넘어갈 수 있게 안내하는 문장을 사용하세요.

        [스타일]
        - 존댓말, 부드럽고 명료하게.
        - 한 번에 너무 많은 질문을 하지 말고, 1~2가지 정도만 묻습니다.
        """
    ...
    try:
        ...
        return "잠시 연결에 문제가 생겼어요. 다시 한 번만 입력해 주세요."

# =========================================================
# 4. UI 렌더링 함수 (실험 준비 페이지 / 헤더 / 메모리 / 채팅)
# =========================================================
def render_scenario():
    st.markdown(
        """
    <div class="scenario-box">
        <b>💡 시나리오 가이드</b><br>
        당신은 <b>헤드셋</b>을 찾고 있습니다. AI에게 원하는 가격, 색상, 기능을 자유롭게 말해보세요.<br>
...
def render_step_header():
    ...
    <div style="font-size:15px; color:#374151; line-height:1.6; margin-bottom:18px;">
        {desc}
    </div>
    """
...
def render_memory_sidebar():
    ...
    <div class="memory-guide-box">
        대화에서 추출된 쇼핑 기준이 이곳에 메모로 쌓입니다.<br>
        기준이 마음에 들지 않으면 <b>직접 삭제</b>하거나 <b>새 기준을 추가</b>할 수 있어요.
    </div>
    """,
        unsafe_allow_html=True,
    )
    ...
    if not st.session_state.memory:
        st.caption("아직 저장된 기준이 없어요. 대화를 하면서 기준을 알려주세요.")
    else:
        for i, mem in enumerate(st.session_state.memory):
            c1, c2 = st.columns([0.85, 0.15])
            with c1:
                st.markdown(
                    f'<div class="memory-block"><span class="memory-text">{naturalize_memory(mem)}</span></div>',
                    unsafe_allow_html=True,
                )
            with c2:
    ...
    new_mem = st.text_input("기준 직접 추가", placeholder="예: 장시간 착용해도 편한 착용감", label_visibility="collapsed")
...
def render_tip_box():
    ...
    <div class="tip-box">
        📝 <b>대화 팁</b><br>
        - 예: "출퇴근용이라 노이즈캔슬링이 중요해요", "예산은 20만 원 정도 생각 중이에요"<br>
        - "추천해줘"라고 말하면, 지금까지 기준을 바탕으로 후보 제품을 보여드려요.
    </div>
    """,
        unsafe_allow_html=True,
    )
...
def recommend_products_ui(name, mems):
    ...
    for i, c in enumerate(products[:3]):
        with cols[i]:
            ...
            if st.button("상세보기", key=f"detail_btn_{i}", use_container_width=True):
                ...
                ai_say(
                    f"**{c['name']}** 제품을 선택하셨군요.\n\n"
                    f"**추천 이유**\n{personalized_reason}\n\n"
                    "배터리, 무게, 착용감 등 궁금한 점이 있다면 자유롭게 물어보세요!"
                )
                log_event("select_product", {"product": c["name"]})
                st.rerun()

    if not st.session_state.comparison_hint_shown:
        ai_say("궁금한 제품의 **상세보기** 버튼을 클릭하면, 그 제품에 대해 더 자세히 질문할 수 있어요 🙂")
...
def handle_input():
    ...
    user_say(user_text.strip())

    # 탐색 단계에서 메모리 추출 + 추천 트리거
    if st.session_state.stage == "explore":
        ...
        if "추천" in user_text:
            ...
            ai_say("지금까지 알려주신 기준을 바탕으로 추천 제품을 가져왔어요 👇")
            log_event("go_comparison", {})
            return

    # 다른 단계든 공통으로 GPT 답변 생성
...
def main_chat_interface():
    ...
    with col2:
            ...
            if st.session_state.stage == "product_detail":
                ...
                with c1:
                    if st.button("⬅️ 목록으로", use_container_width=True):
                        ...
                        log_event("back_to_list", {})
                        st.rerun()
                with c2:
                    if st.button("🛒 이 제품으로 결정하기", use_container_width=True):
                        ...
                        log_event("purchase_decision_click", {"product": st.session_state.final_choice})
                        st.rerun()

            # 추천 카드 영역 (비교 / 상세 / 결정 단계 모두에서 노출)
        ...
        if st.session_state.stage == "purchase_decision" and st.session_state.selected_product:
            ...
            st.success(f"🎉 **{p['name']}** 구매를 결정하셨습니다! (실제 구매는 진행되지 않습니다)")
            st.balloons()
        ...
        with st.form(key="chat_form", clear_on_submit=True):
            ...
            with c1:
                st.text_input(
                    ...
                    placeholder="헤드셋 용도, 예산, 원하는 기능 등을 자유롭게 입력해 주세요...",
                )
            with c2:
...
if st.session_state.page == "context_setting":
    ...
    <div class="info-text">
        ...
        평소 본인의 실제 쇼핑 습관이나, 이번 실험에서 연기할 <b>'페르소나'</b>의 정보를 입력해 주세요.
    </div>
    """,
        unsafe_allow_html=True,
    )
    ...
    with st.container(border=True):
        ...
        with c1:
            ...
            st.markdown(
                ...
                "⚠️ 사전 설문에 작성한 이름과 동일하게 입력해주세요. (불일치 시 불성실 응답 간주 가능)"
                "</div>",
                unsafe_allow_html=True,
            )
        with c2:
        ...
        category = st.selectbox(
            "Q1. 최근에 가장 많이 구매한 상품 카테고리는 무엇인가요?",
            ["패션/의류", "디지털/가전", "생활용품", "뷰티", "식품", "기타"],
        )
        ...
        criteria = st.selectbox(
            "Q3. 해당 제품 구매 시 가장 중요하게 생각한 기준은?",
            ["디자인/색상", "가격/가성비", "성능/스펙", "브랜드 인지도", "사용자 리뷰/평점"],
        )
        ...
        if st.button("쇼핑 시작하기 (정보 저장)", type="primary", use_container_width=True):
            if name and recent_item and fav_color:
                ...
                # 첫 인사 고정 멘트
                fixed_greeting = (
                    f"안녕하세요 {name}님! 😊 저는 당신의 AI 쇼핑 도우미예요.\n"
                    "대화를 통해 고객님의 쇼핑 기준을 기억하면서, 함께 헤드셋을 찾아볼게요.\n"
                    "먼저, 헤드셋을 주로 어떤 상황에서 사용하실 예정인가요?"
                )
                ai_say(fixed_greeting)

                log_event("start_chat", {"name": name, "recent_item": recent_item})
                st.rerun()
            else:
