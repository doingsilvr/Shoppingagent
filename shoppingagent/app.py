...
def generate_personalized_reason(product, mems, name):
    ...
    카드용 요약을 2줄로 생성하되,
    - 1줄: 제품 특성 기반 핵심 요약
    - 2줄: 사용자의 메모리(기준)과 자연스럽게 연결한 개인화 문장
    """
    mem_str = " ".join(mems)
    ...
    if "음질" in tags:
        product_line = "음질 평가가 좋은 편이에요."
    if "노이즈캔슬링" in tags:
        product_line = "노이즈캔슬링 성능이 괜찮은 제품이에요."
    if "편안함" in tags or "착용감" in tags:
        product_line = "착용감이 편하다는 평가가 많아요."
    if "가성비" in tags:
        product_line = "동급 대비 가성비가 뛰어난 모델이에요."
    if "배터리" in tags:
        product_line = "배터리가 오래가는 편이에요."
    if product_line is None:
        product_line = "전반적으로 균형 잡힌 성능을 가진 제품이에요."

    # ----------------------------
    # 2️⃣ 메모리(기준) 기반 개인화 문장
    # ----------------------------
    personalization = None

    # 음질
    if "음질" in mem_str:
        personalization = "기억하기로는 음질을 중요하게 보셨던 것 같아서, 잘 어울릴 거예요."
    # 착용감
    elif "착용감" in mem_str:
        personalization = "장시간 착용 기준을 언급하셨던 만큼, 착용감이 잘 맞는 제품이에요."
    # 노캔
    elif "노이즈캔슬링" in mem_str:
        personalization = "노이즈캔슬링을 신경 쓰셨던 점을 고려하면 더 만족도가 높을 거예요."
    # 예산
    elif "예산" in mem_str or "만원" in mem_str:
        personalization = "기억해둔 예산 범위 안에서 가장 효율적인 선택지예요."
    # 무게/경량
    elif "가벼운" in mem_str or "무게" in mem_str:
        personalization = "가벼운 제품을 선호하신다고 하셔서 더 적합해 보여요."
    # 색상
    elif "색상" in mem_str:
        personalization = "색상에 대한 선호도도 잘 반영되는 모델이에요."
    else:
        personalization = "말씀하신 기준들에 자연스럽게 맞닿아 있는 제품이에요."
