...
- 이미 음질/착용감/노이즈캔슬링/디자인/배터리 등 을 한 번 물어본 적이 있다면 다시 묻지 않는다.
...
- 메모리와 최신 발화가 충돌하면 “예전에 말씀해주신 내용과 조금 다른데, 이번에는 새 기준을 우선해서 반영할까요?”라고 확인한다.
...
def context_setting_page():
    ...
    with st.container(border=True):
        ...
        if st.button("쇼핑 시작하기(여러번 클릭)", type="primary", use_container_width=True):
            ...
            elif shopping_style == "디자인/스타일 우선":
