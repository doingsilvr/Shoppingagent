...
- **사용자는 블루투스 '헤드셋(오버이어/온이어)'을 구매하려고 한다. '이어폰' 또는 '인이어' 타입에 대한 질문은 피하라.** (📌 3번 문제 해결)

[대화 흐름 규칙]
- 대화 초반에는 사용 용도/상황 → 기능/착용감/배터리/디자인/브랜드/색상 → 예산 순으로 자연스럽게 넓혀 간다.
- **(📌 2번 문제 해결) 메모리에 이미 용도/상황/기능 등의 기준이 파악되었다면, 다음 단계의 질문으로 넘어가라.**
...
def _clause_split(u: str) -> list[str]:
    # 📌 1번 문제 해결: 다양한 접속사(및, 하고, 고, & 등)를 쉼표로 변환하여 복수의 기준을 분리
...
def memory_sentences_from_user_text(utter: str):
    ...
    # 2) 브랜드
    brands = ["Sony", "BOSE", "Bose", "JBL", "Apple", "Anker", "Soundcore", "
