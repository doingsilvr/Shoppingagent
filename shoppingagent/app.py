...
# =========================================================
# 🔥 7) 모든 탐색 단계의 응답은 반드시 "질문으로 끝나도록" 보정한다
# =========================================================
if stage == "explore":

    # 질문 패턴이 없으면 자동으로 질문을 붙여준다
    question_endings = ["?", "까요", "가요", "신가요", "어떨까요", "어떤가요"]

    if not any(reply.strip().endswith(q) for q in question_endings):

        # 기준이 부족하면 → 기준을 더 요청하는 질문
        if len(ss.memory) < 3:
            reply = reply.rstrip("。.!") + "\n\n혹시 더 중요하게 생각하시는 기준이 있으실까요?"
        else:
            # 기준이 어느 정도 모인 경우 → 자연스러운 후속 질문
            reply = reply.rstrip("。.!") + "\n\n이 부분과 관련해서 더 선호하시는 기준이 있을까요?"

# =========================================================
# 🔥 8) “기준이 충분해야 추천 가능” 문구를 자연스럽게 포함시키기
# =========================================================
if stage == "explore":

    # 1) 질문이 없으면 자동으로 질문 추가
    question_endings = ["?", "까요", "가요", "신가요", "어떨까요", "어떤가요"]
    if not any(reply.strip().endswith(q) for q in question_endings):

        # 기준 부족 → 더 많은 기준 요청
        if len(ss.memory) < 3:
            reply = reply.rstrip("!.") + "\n\n혹시 더 중요하게 생각하시는 기준이 있을까요?"
        else:
            reply = reply.rstrip("!.") + "\n\n이 기준과 관련해서 더 선호하시는 요소가 있으실까요?"

    # 2) 기준 부족/중간 안내
    if len(ss.memory) < 3:
        reply += "\n\n(참고로, 몇 가지 기준이 더 모이면 더 정확한 추천이 가능해요 😊)"
    elif 3 <= len(ss.memory) < 5:
        reply += "\n\n(이제 거의 다 왔어요! 조금만 더 알려주시면 추천해드릴 수 있어요 😊)"
    ...
    if stage == "explore":
        ...
        # 1) 음질 질문 반복 방지
        if any(k in reply for k in ["음질", "사운드", "소리", "착용감", "배터리"]) and "sound" in ss.question_history:
            return "관련 선호도는 이미 알고 있어요! 다음 기준으로 넘어가볼게요 😊"
