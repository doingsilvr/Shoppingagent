...
def gpt_reply(user_input: str) -> str:
    ...
    # 🚨 이전 구매 내역 컨텍스트를 GPT에게 힌트로 제공 (초기 단계에서만)
    initial_context = ""
    if st.session_state.initial_purchase_context and len(st.session_state.memory) < 3: 
        initial_context = f"[이전 구매 내역] 사용자가 최근 '{st.session_state.initial_purchase_context}'를 구매했음을 기억하고, 이와 관련된 취향(색상, 디자인, 가격 등)을 파악하는 대화에 집중하세요. 이후 블루투스 헤드셋 기준으로 전환하세요.\n"
...
def chat_interface():
    ...
    # 🚨 첫 인사 / 초기 질문 유도 (과거 구매 맥락 질문 포함)
    if not st.session_state.messages:
        purchase_context = st.session_state.initial_purchase_context
        
        if purchase_context:
            # 🚨 1차 질문: 과거 구매 맥락 질문 (대화 유도)
            ai_say(
                f"안녕하세요 {st.session_state.nickname}님! 😊 저는 당신의 AI 쇼핑 도우미예요. "
                f"최근 '{purchase_context}'를 구매하신 것을 참고하여, {st.session_state.nickname}님의 평소 취향을 먼저 파악해볼게요. "
                f"그 '{purchase_context}'를 구매하실 때 **'디자인'**이나 **'가격'**, **'성능/품질'** 중에서 어떤 점을 가장 중요하게 생각하셨나요? **색상**이나 **재질** 같은 구체적인 점도 좋아요."
            )
            st.rerun() 
        else:
            # Default start question (should not happen if onboarding is done)
            ai_say(
                f"안녕하세요 {st.session_state.nickname}님! 😊 저는 당신의 AI 쇼핑 도우미예요. "
...
def context_setting():
    ...
    st.markdown("---")
    # 🚨 질문 수정: 하나만 묻도록 변경
    st.markdown("#### 질문: 최근 3개월 동안 어떤 제품(카테고리)을 구매하셨나요? (하나만 적어주세요)")
    st.caption("예: 옷, 신발, 시계, 태블릿 등. 이 정보는 대화 시작 시 취향 파악에 활용됩니다.")
    
    purchase_list = st.text_input("최근 구매 품목 (1가지)", placeholder="예: 옷", key="purchase_list_input") 
    
    # 🚨 기존 구매 기준을 묻는 라디오 버튼 제거
    
    if st.button("AI와 대화 시작"):
        if not purchase_list.strip():
