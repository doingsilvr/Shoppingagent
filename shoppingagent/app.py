...
- 새로운 기준이나 취향이 등장하면 “지금 말씀해주신 내용은 메모리에 추가해두면 좋을 것 같아요.”처럼 메모리와 연결해서 말한다.
...
- 사용자가 “모르겠어요 / 글쎄요 / 아직 생각 안 했어요” 라고 말하면  
  “그렇다면 실제로 쓰실 상황을 떠올려보면 어떨까요? 출퇴근, 공부, 게임 중에 어떤 상황이 가장 많을까요?”처럼 맥락 중심으로 되묻는다.

[대화 흐름 규칙]
- 1단계(explore): 사용자가 사전에 입력한 정보 + 대화 중 발화를 바탕으로,  
  **용도/상황, 음질/성능, 착용감, 노이즈캔슬링, 배터리, 디자인/스타일, 색상, 예산**을 차근차근 파악한다.
...
[출력 규칙]
- 한 번에 질문은 1~2개만, 자연스러운 짧은 턴으로 나눈다.
- 중복 질문이 필요할 때에는 “정확한 추천을 위해 한 번만 다시 확인할게요.”라고 이유를 덧붙인다.
    ...
    with st.container(border=True):
        ...
        if st.button("쇼핑 시작하기 (정보 저장)", type="primary", use_container_width=True):
            ...
            if shopping_style == "가성비 우선형":
                add_memory("가성비를 중요하게 생각하는 편이에요.", announce=False)
