...
# =========================================================
SYSTEM_PROMPT = r"""
너는 'AI 쇼핑 도우미'이며 사용자의 블루투스 헤드폰 기준을 파악해 추천을 돕는 역할을 한다.
...
- 사용자는 블루투스 헤드폰을 구매하려고 한다. 이어폰이나 인이어 타입에 대한 질문은 하지 않는다.

[대화 흐름 규칙]
- 1단계: 초기 대화에서는 사용자가 사전에 입력한 정보(중요 기준, 선호 색상)를 바탕으로 사용자 취향을 파악한다.
- 2단계: 사용자 취향이 1~2개 파악되면, 구매 목표인 블루투스 헤드폰 기준을 순서대로 질문한다. 
...
def recommend_products(name, mems, is_reroll=False):
    ...
    # 하단 안내 문구
    ai_say("\n궁금한 제품의 상세 보기 버튼을 클릭해 해당 제품에 대해 궁금한 점을 질문해 주세요.🙂")
...
def top_memory_panel():
    with st.container():
        ...
        new_mem = st.text_input(
            "새 메모리 추가",
            placeholder="예: 노이즈캔슬링 필요 / 음질 중요하게 생각함",
...
def chat_interface():
    ...
    if len(st.session_state.messages) == 0:
        ai_say(
            ...
            "대화를 통해 고객님의 정보를 기억하며 함께 헤드폰을 찾아볼게요. "
...
def context_setting():
    st.markdown("### 🧾 실험 준비 ")
    st.caption("헤드폰 구매에 반영될 기본 정보와 평소 취향을 간단히 입력해 주세요. 이후 실험은 과거에도 대화한 내역이 있다는 가정 하에 진행되기 때문에 해당 내용은 과거 대화 속 습득한 정보로 기억될 예정입니다.")
