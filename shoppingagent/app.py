        ...
        with cols[i]:
            ...
            if st.button(f"후보 {i+1} 상세 정보 보기", key=f"detail_btn_{i}"):
                detail_block = (
                    ...
                    f"\n📌 이 상품에 대해 어떤 점이 궁금한지 알려주세요!(예 : 부정적인 리뷰는 어떤 내용이야? / 배터리 성능은 어때?)"
                )
                ai_say(detail_block)
                st.rerun()
...
이 단계에서는 **기준을 다시 묻지 않고**, 사용자가 선택한 제품에 대해 필요한 정보를 명확하게 제공합니다.
...
• 가격: {product['price']:,}원
• 평점: {product['rating']}점 / 리뷰 {product['reviews']}개
• 주요 특징: {', '.join(product['tags'])}
• 리뷰 요약: {product['review_one']}
• 색상 옵션: {', '.join(product['color'])}

[응답 규칙]
1) 사용자의 질문에 대해 제품 정보에 기반해 정확하게 답하고,
2) 기준을 새로 묻지 않으며 (“어떤 점이 더 중요하신가요?” 금지),
3) 아래와 같은 ‘더 궁금할 만한 예시’를 1~2개 보여주며 자연스럽게 다음 질문을 유도합니다.
4) 만일 제품 정보로 파악하기 어려운 질문이라면, 카탈로그에 없더라도 직접 제품 정보에 기반해 내용을 만들어서 제공합니다.
...
이제 위 내용을 기반으로 자연스럽고 친절하게 답변하세요.
"""
