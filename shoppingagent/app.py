...
def ss_init():
    ...
    # 쇼핑 진행 단계 (explore -> comparison -> product_detail -> purchase_decision)
    ...
    ss.setdefault("recommended_products", []) # 중복 추천 방지 기록용
...
# =========================================================
# 2. CSS 스타일 (디자인: 파란 버튼, 2단 레이아웃, 가로 진행바)
# =========================================================
st.markdown("""
<style>
    /* 상단 헤더, 푸터 숨김 */
    ...
    div.stButton > button:hover {
        background-color: #1D4ED8 !important; /* 호버 시 진한 파랑 */
    }
    ...
    .chat-bubble-user { 
        background: #E0E7FF; align-self: flex-end; margin-left: auto; 
...
def extract_budget(mems):
    ...
    for m in mems:
        ...
        # 숫자 추출 시도 (정교한 정규식)
        m_val = re.search(r"(\d+)\s*만\s*원", m)
        if m_val: return int(m_val.group(1)) * 10000
        
        m_val2 = re.search(r"(\d{3,})\s*원", m.replace(",", ""))
        if m_val2: return int(m_val2.group(1))
...
CATALOG = [
    ...
    {"name": "Bose QC45", "brand": "Bose", "price": 420000, "rating": 4.7, "reviews": 2800, "rank": 2, "tags": ["가벼움", "착용감", "노이즈캔슬링", "편안함"], "review_one": "장시간 써도 귀가 편하다는 리뷰가 많아요.", "color": ["블랙", "화이트"], "img": "https://raw.githubusercontent.com/doingsilvr/Shoppingagent/main/shoppingagent/img/Bose%20QC45.jpg"},
    {"name": "Sony WH-1000XM5", "brand": "Sony", "price": 450000, "rating": 4.8, "reviews": 3200, "rank": 1, "tags": ["노이즈캔슬링", "음질", "착용감", "통화품질"], "review_one": "소음 많은 환경에서 확실히 조용해진다는 평가.", "color": ["핑크", "실버", "블랙"], "img": "https://raw.githubusercontent.com/doingsilvr/Shoppingagent/main/shoppingagent/img/Sony%20WH-1000XM5.jpg"},
    {"name": "Apple AirPods Max", "brand": "Apple", "price": 679000, "rating": 4.6, "reviews": 1500, "rank": 3, "tags": ["브랜드", "노이즈캔슬링", "디자인", "고급"], "review_one": "깔끔한 디자인과 가벼운 무게로 만족도가 높아요.", "color": ["실버", "스페이스그레이", "핑크"], "img": "https://raw.githubusercontent.com/doingsilvr/Shoppingagent/main/shoppingagent/img/Apple%20Airpods%20Max.jpeg"},
...
def filter_products(mems, is_reroll=False):
    ...
    def score(c):
        s = c["rating"] * 2  # 기본 점수 (10점 만점 환산)
        
        # 1. 예산 필터링 및 점수
        if budget_val:
            if c["price"] > budget_val * 1.5: return -999 # 예산 초과 제외
        ...
        for m in mems:
            if "색상" in m:
                ...
                elif "실버" in m: preferred_color = "실버"
