...
def ss_init():
    ...
    # 메모리 충돌 처리용 상태
    ss.setdefault("pending_memory_conflict", None)  # 기존 충돌 메모리 리스트
    ss.setdefault("pending_memory_new", None)       # 새로 추가하려던 메모리
...
# =========================================================
# SYSTEM PROMPT (헤드셋 전용 + 메모리/투명성 강조)
# =========================================================
SYSTEM_PROMPT = r"""
...
- (중요) 메모리가 6개 이상이면 "지금까지 메모리를 한 번 정리해드릴까요?"라고 말하며, 요약/추천 단계로 넘어갈 수 있음을 알려준다.
...
- 사용자가 특정 상품 번호를 물어보면 그 제품의 특징, 장단점, 리뷰 요약 등을 제공하고, 사용자의 메모리 기준을 반영해 개인화된 설명을 덧붙인다.

[메모리 활용]
- 메모리에 저장된 기준을 항상 반영해 대화를 이어간다.
- 메모리와 사용자의 최신 발언이 충돌하면 
  "기존에 ~라고 하셨는데, 기준을 바꾸실까요? 아니면 둘 다 고려해드릴까요?"라고 정중히 확인한다.
- 요약(summary) 단계에서는 "지금까지 제가 메모리에 저장해 둔 내용"을 사용자에게 투명하게 공유한다.
- 추천이나 제품 설명을 할 때는, 가능하면 메모리 기준 중 1~2개를 골라 연결해서 설명한다.
  예: "메모리에 따르면 ~님은 ○○를 중요하게 보셨는데, 이 제품이 그 부분을 잘 충족해요."

[출력 규칙]
- 한 번에 너무 많은 질문을 하지 않고 자연스럽게 한두 개씩 묻는다.
- 중복 질문은 피하며 꼭 필요한 경우 "다시 한번만 확인할게요"라고 말한다.
- 전체 톤은 부드러운 존댓말을 유지하되, 캐주얼하고 편안한 말투를 사용한다.
- 항상 이 대화는 '블루투스 헤드셋 쇼핑'에 대한 대화라는 것을 잊지 말고, 다른 기기 카테고리로 벗어나지 않는다.
"""
...
def add_memory(mem_text: str, announce=True):
    ...
    # 예산 카테고리 충돌 제거 (새로운 예산이 들어오면 기존 예산 삭제)
    if "예산은 약" in mem_text_stripped:
        st.session_state.memory = [m for m in st.session_state.memory if "예산" not in m]

    # 색상 카테고리 충돌 → 바로 덮어쓰지 않고, 사용자에게 한번 물어보기
    if _is_color_memory(mem_text_stripped):
        existing_colors = [m for m in st.session_state.memory if _is_color_memory(m)]
        if existing_colors:
            # 충돌 상태 저장
            st.session_state.pending_memory_conflict = existing_colors
            st.session_state.pending_memory_new = mem_text
            st.session_state.notification_message = "🎨 색상 관련 메모리가 겹쳐서, 어떻게 할지 한 번 여쭤볼게요."
            # 질문 한 번 던지기 (ai_say는 아래에서 정의되지만, 런타임 순서상 문제 없음)
            question = (
                f"이미 메모리에 '{naturalize_memory(existing_colors[0])}'라고 저장되어 있는데, "
                f"새로 '{naturalize_memory(mem_text)}'라고 말씀해주셨어요.\n\n"
                f"색상 기준을 **둘 다 유지할까요, 아니면 새 기준으로 바꿀까요?**\n"
                f"- 예: 둘 다 유지할래요 / 새 색으로 바꿔줘"
            )
            ai_say(question)
            return
...
def delete_memory(idx: int):
    if 0 <= idx < len(st.session_state.memory):
        ...
        if st.session_state.page != "context_setting":
            st.session_state.notification_message = "🧹 메모리에서 내용을 삭제했어요."
        st.session_state.memory_changed = True
...
def generate_summary(name, mems):
    ...
    if not mems:
        return (
            f"아직 {name}님에 대해 메모리에 저장된 정보가 많지 않아요.\n"
            "대화를 조금 더 이어가면서 어떤 헤드셋을 좋아하실지 같이 찾아가볼게요."
        )
    lines = "\n".join([f"- {naturalize_memory(m)}" for m in mems])
    return (
        f"지금까지 {name}님과의 대화를 바탕으로 제가 **메모리에 저장해 둔 내용**은 다음과 같아요.\n"
        f"이 메모리는 {name}님이 어떤 블루투스 헤드셋을 선호하시는지 더 정확하게 이해하고,\n"
        f"추천 과정에 투명하게 반영하기 위해 사용돼요.\n\n"
        f"{lines}\n\n"
        f"이 메모리를 바탕으로 다음 단계에서는 {name}님에게 잘 맞을 것 같은 헤드셋 후보들을 골라보고,\n"
        f"각 제품이 어떤 메모리 기준을 반영한 선택인지도 함께 설명드릴게요."
    )

def generate_personalized_reason(product, mems, name):
    """제품 추천 시, 메모리 기반 개인화 이유 텍스트 생성"""
    mem_str = " ".join(mems)
    ...
    if "음질" in mem_str and ("음질" in tags_str or "균형 음질" in tags_str):
        reasons.append("중요하게 말씀해 주셨던 **음질** 만족도가 높은 제품이에요.")
        used_mems += [m for m in mems if "음질" in m]
    if any(k in mem_str for k in ["착용감", "편안"]) and any(t in tags_str for t in ["편안함", "가벼움", "경량", "착용감"]):
        reasons.append("장시간 착용해도 편한 **착용감**이 강점이에요.")
        used_mems += [m for m in mems if "착용감" in m or "편안" in m]
    if "노이즈캔슬링" in mem_str and "노이즈캔슬링" in tags_str:
        reasons.append("소음 많은 환경에서도 도움이 되는 **노이즈캔슬링** 성능이 좋아요.")
        used_mems += [m for m in mems if "노이즈캔슬링" in m or "노이즈" in m]
    if "디자인" in mem_str or "스타일" in mem_str:
        if "디자인" in tags_str:
            reasons.append("외관이나 **디자인/스타일** 면에서도 잘 맞을 가능성이 커요.")
            used_mems += [m for m in mems if "디자인" in m or "스타일" in m]

    # 중복 제거 + 최대 3개까지만
    seen = set()
    unique_used = []
    for m in used_mems:
        nm = naturalize_memory(m)
        if nm not in seen:
            seen.add(nm)
            unique_used.append(nm)
    unique_used = unique_used[:3]

    base_text = ""
    if reasons:
        base_text = " ".join(reasons)
    else:
        base_text = f"{name}님의 취향과 전반적으로 잘 맞는 인기 제품이에요."

    if unique_used:
        mem_lines = "\n".join([f"- {m}" for m in unique_used])
        tail = (
            f"\n\n제가 기억하고 있는 {name}님의 메모리 기준 중에서 이 제품과 특히 잘 맞는 내용은 다음이에요:\n"
            f"{mem_lines}\n\n"
            f"이런 점들을 고려하면 실제로 사용해보셨을 때 만족도가 꽤 높을 가능성이 커 보여요."
        )
        return base_text + tail
    else:
        return base_text
...
def recommend_products(name, mems, is_reroll=False):
    ...
    st.markdown(
        "지금까지 대화에서 쌓인 **메모리**를 바탕으로 고른 상위 3개 제품이에요. "
        "궁금한 제품의 ‘상세 정보 보기’를 눌러서 더 자세히 물어보실 수 있어요.\n"
    )
    ...
    for i, c in enumerate(products[:3]):
        ...
        with cols[i]:
            ...
            if st.button("상세 정보 보기", key=f"detail_btn_{i}", use_container_width=True):
                ...
                detail_block = (
                    f"**{c['name']} ({c['brand']})** 제품을 자세히 볼게요!\n\n"
                    f"- 가격: {c['price']:,}원\n"
                    f"- 평점: {c['rating']:.1f} / 5.0\n"
                    f"- 색상: {', '.join(c['color'])}\n"
                    f"- 한 줄 리뷰: {c['review_one']}\n\n"
                    f"**메모리 기반 추천 이유**\n"
                    f"{personalized_reason}\n\n"
                    f"배터리, 무게, 통화 품질 같은 구체적인 부분이 궁금하시다면 편하게 물어봐 주세요 🙂"
                )
                ai_say(detail_block)
                st.rerun()
                return
    ...
    if not st.session_state.comparison_hint_shown:
        ai_say("\n궁금한 제품의 상세 보기 버튼을 클릭하면, 메모리를 반영해서 더 자세히 설명해 드릴게요 🙂")
...
def get_product_detail_prompt(product, user_input):
    ...
    if not memory_text:
        memory_text_display = "(아직 메모리에 저장된 내용이 많지 않습니다.)"
    else:
        memory_text_display = memory_text
...
{budget_rule}5. 가능하다면 위 메모리 중 1가지 이상을 자연스럽게 언급하며,
   왜 이 제품이 사용자의 취향과 잘 맞는지 설명하세요.
   예: "메모리에 따르면 ~님은 ○○를 중요하게 보셨는데, 이 제품이 그 부분을 잘 충족해요."
...
def gpt_reply(user_input: str) -> str:
    ...
    # 1) product_detail 단계: 전용 프롬프트 + SYSTEM PROMPT 함께 사용
...
[현재까지 저장된 쇼핑 메모리]
{memory_text if memory_text else "아직 메모리에 저장된 내용이 없습니다."}

[사용자 발화]
{user_input}

위 정보를 참고해, 블루투스 헤드셋 쇼핑 도우미로서 다음 말을 한국어 존댓말로 자연스럽게 이어가세요.
가능하다면 "제가 기억하기로는 ~", "메모리에 따르면 ~" 같은 표현을 사용해,
메모리를 활용하고 있다는 느낌을 자연스럽게 드러내 주세요.
"""
...
def handle_user_input(user_input: str):
    ...
    # 0-0) 색상 메모리 충돌에 대한 사용자 응답 처리
    if st.session_state.pending_memory_conflict and st.session_state.pending_memory_new:
        lower = user_input.lower()
        # 둘 다 유지
        if "둘 다" in lower or "둘다" in lower or "같이" in lower:
            new_mem = st.session_state.pending_memory_new
            # 기존 색상 메모리는 그대로 두고, 새 메모리 추가
            if new_mem not in st.session_state.memory:
                st.session_state.memory.append(new_mem)
            st.session_state.pending_memory_conflict = None
            st.session_state.pending_memory_new = None
            ai_say("네, 기존 색상 기준과 새로 말씀해 주신 색상 기준을 둘 다 메모리에 함께 유지해 둘게요.")
            st.session_state.notification_message = "🎨 색상 메모리가 둘 다 유지되도록 반영되었어요."
            st.session_state.memory_changed = True
            st.rerun()
            return
        # 새 기준으로 변경
        elif any(k in lower for k in ["바꿔", "새로", "이걸로", "이번 색", "이번색"]):
            new_mem = st.session_state.pending_memory_new
            # 기존 색상 메모리 제거 후 새 기준으로 교체
            st.session_state.memory = [m for m in st.session_state.memory if not _is_color_memory(m)]
            st.session_state.memory.append(new_mem)
            st.session_state.pending_memory_conflict = None
            st.session_state.pending_memory_new = None
            ai_say("알겠습니다. 색상 기준은 새로 말씀해 주신 내용으로 메모리를 업데이트해 둘게요.")
            st.session_state.notification_message = "🎨 색상 메모리가 새 기준으로 업데이트되었어요."
            st.session_state.memory_changed = True
            st.rerun()
            return
        else:
            ai_say(
                "색상 기준을 어떻게 할지 아직 확실히 이해하지 못했어요.\n"
                "아래 중에서 편하게 말씀해 주세요.\n"
                "- 둘 다 유지할래요\n"
                "- 새 색으로 바꿔줘"
            )
    ...
    if product_re and st.session_state.stage == "comparison":
        ...
        if 0 <= idx < len(st.session_state.current_recommendation):
            ...
            st.session_state.stage = "product_detail"
            st.session_state.product_detail_turn = 0
            # 제품 상세 기본 설명 + 이후 질문은 product_detail에서 처리
            c = st.session_state.selected_product
            personalized_reason = generate_personalized_reason(c, st.session_state.memory, st.session_state.nickname)
            detail_block = (
                f"**{c['name']} ({c['brand']})** 제품을 선택하셨군요!\n\n"
                f"- 가격: {c['price']:,}원\n"
                f"- 평점: {c['rating']:.1f} / 5.0\n"
                f"- 색상: {', '.join(c['color'])}\n"
                f"- 한 줄 리뷰: {c['review_one']}\n\n"
                f"**메모리 기반 추천 이유**\n"
                f"{personalized_reason}\n\n"
                f"배터리, 무게, 통화 품질 같은 구체적인 부분이 궁금하시면 편하게 물어봐 주세요 🙂"
            )
            ai_say(detail_block)
            st.rerun()
            return
    ...
    if st.session_state.stage == "explore":
        ...
        if mem_count >= 6 and has_budget:
            ai_say("지금까지 말씀해주신 내용을 한 번 정리해보고, 그 메모리를 바탕으로 헤드셋을 추천해볼게요.")
    ...
    if any(k in user_input for k in ["추천해줘", "추천 좀", "골라줘", "추천 부탁", "추천", "후보 보여줘"]):
        if extract_budget(st.session_state.memory) is None:
            ...
            st.rerun()
            return
        ai_say("알겠습니다. 지금까지의 메모리를 정리한 뒤, 그 내용을 바탕으로 헤드셋 후보들을 보여드릴게요.")
        st.session_state.stage = "summary"
        summary_step()
        st.rerun()
        return
    ...
    if any(k in user_input for k in ["없어", "그만", "끝", "충분"]):
        ...
        if extract_budget(st.session_state.memory) is None:
            ...
            st.rerun()
            return
        ai_say("알겠습니다. 지금까지의 메모리를 정리한 뒤, 그 내용을 바탕으로 추천을 이어가볼게요.")
        st.session_state.stage = "summary"
        summary_step()
        st.rerun()
        return
    ...
    if st.session_state.stage == "summary":
        ai_say("위에 정리된 메모리를 한 번 확인해보시고, 아래 버튼으로 추천을 받아보셔도 좋아요 🙂")
        st.rerun()
        return
...
def top_memory_panel():
    with st.container():
        if len(st.session_state.memory) == 0:
            st.caption("아직 파악된 정보가 없습니다. 대화 중에 메모리가 차곡차곡 쌓일 거예요.")
        else:
            for i, item in enumerate(st.session_state.memory):
                cols = st.columns([7, 1])
                with cols[0]:
                    display_text = naturalize_memory(item)
                    st.markdown(f"**메모리 {i+1}.**", help=item, unsafe_allow_html=True)
                    st.markdown(
...
def chat_interface():
    ...
    if len(st.session_state.messages) == 0:
        ai_say(
            ...
            "대화를 통해 {nickname}님의 취향을 메모리에 하나씩 쌓아가면서 함께 헤드셋을 찾아볼게요. "
            "먼저, 주로 어떤 용도로 사용할 예정인지 알려주실 수 있을까요?"
        )
    ...
    with col_chat:
        ...
        if st.session_state.stage == "summary":
            if st.button("🔍 메모리 기반으로 추천 받아보기", key="go_reco_button", use_container_width=True):
                st.session_state.stage = "comparison"
                st.rerun()
        ...
        with st.form(key="chat_form_main", clear_on_submit=True):
            user_text = st.text_area(
                "",
                placeholder="원하는 기준이나 궁금한 점을 알려주세요! (예: 노이즈캔슬링이 중요해요 / 화이트 색상 좋아해요)",
                height=80,
            )
