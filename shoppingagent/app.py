...
- 2단계: 사용자 취향이 1~2개 파악되면, 구매 목표인 블루투스 헤드셋 기준을 순서대로 질문한다. 
  순서: 용도/사용 상황 -> 기능/착용감/배터리/디자인/브랜드/인지도/색상 -> 예산.
...
def memory_sentences_from_user_text(utter: str):
    ...
    for c in clauses:
        base_rules = [
            ...
            ("유명한", "브랜드 인지도가 높은 제품을 선호하고 있어요."),
            ("인지도", "브랜드 인지도를 중요하게 생각하고 있어요."),
        ]
        matched = False
...
def gpt_reply(user_input: str) -> str:
    ...
    if st.session_state.stage == "explore":
        ...
        if is_design_in_memory and not is_color_in_memory:
            stage_hint += (
                "[디자인 기준이 이미 파악되었습니다. 이번 턴에서는 반드시 디자인 취향(미니멀/레트로/깔끔한 등) "
                "또는 선호 색상에 대한 질문을 하세요.]"
            )
    ...
    def ask_brand_preference_if_needed():
        for m in st.session_state.memory:
            if "브랜드" in m or "인지도" in m or "유명" in m:
                ...
                return (
                    "브랜드 인지도를 중요하게 보고 계시는군요! 😊\n"
                    "혹시 선호하거나 신뢰하는 브랜드가 있나요?\n"
