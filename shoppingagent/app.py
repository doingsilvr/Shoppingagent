...
st.set_page_config(
    ...
    page_icon="🎧",
    layout="centered"  # iframe 안에서 덜 잘리도록 중앙 정렬
)

# 메인 컨테이너 폭 / 카드 스타일 / 채팅 버블 스타일
st.markdown(
    """
    <style>
    /* 전체 영역 폭 줄이기 (iframe 안에서도 덜 잘리도록) */
...
def ss_init():
    ...
    ss.setdefault("page", "context_setting")  # 온보딩 제거, 바로 사전 정보 입력 -> chat
...
def generate_summary(name, mems):
    ...
    tail = (
        "\n제가 정리한 기준이 맞을까요? 좌측 메모리 제어창에서 언제든 수정할 수 있어요.\n"
...
def chat_interface():
    ...
    user_input = None  # chat_input 값을 컬럼 밖에서도 쓰기 위해 초기화
    ...
    # ===== 컬럼 렌더링 이후에 입력 처리 & 메모리 알림 노출 =====
    if user_input:
        user_say(user_input)
        handle_user_input(user_input)

    # 메모리 알림은 입력 처리 후 한 번에 노출 (한 턴 늦는 문제 방지)
