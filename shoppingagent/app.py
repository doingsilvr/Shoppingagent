...
[대화 흐름 규칙]
- **🚨 [초기 메모리 반영] 2단계에서 입력된 기존 구매 내역의 취향 정보를 바탕으로 대화를 시작해야 한다.**
...
def ss_init():
    ...
    ss.setdefault("page", "onboarding")       # 1. 이름 입력
    ss.setdefault("stage", "explore")         # 3. 채팅 시작
...
def context_setting():
    st.title("💡 실험 준비: 초기 취향 정보 수집")
    st.caption(f"헤드셋 구매 전, {st.session_state.nickname}님의 평소 취향을 파악합니다.")
    
    st.markdown("---")
    
    # 🚨 질문 1: 구매 품목 입력
    st.markdown("#### 1. 최근 3개월 동안 어떤 제품(카테고리)을 구매하셨나요? (하나만 적어주세요)")
    st.caption("예: 옷, 신발, 태블릿 등")
    purchase_list = st.text_input("최근 구매 품목", placeholder="예: 옷", key="purchase_list_input") 
    
    # 🚨 질문 2: 색상 입력
    st.markdown("#### 2. 그 제품의 선호했던 색상은 무엇인가요?")
    color_option = st.text_input("선호 색상", placeholder="예: 화이트 / 블랙", key="color_input") 
    
    # 🚨 질문 3: 중요 기준 입력 (라디오 버튼)
    st.markdown("#### 3. 해당 품목을 구매할 때, 다음 중 어떤 점을 가장 중요하게 고려했나요?")
    ...
    if st.button("AI와 대화 시작"):
        ...
        # 🚨 메모리 주입 (메모리 형식에 맞춰 저장)
        add_memory(f"최근 구매한 {purchase_list.strip()}의 색상은 {color_option.strip()}입니다.", announce=False)
        add_memory(f"(가장 중요) 최근 구매한 {purchase_list.strip()}의 중요 기준은 {priority_option}입니다.", announce=False)
