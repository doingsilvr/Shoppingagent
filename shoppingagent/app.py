...
def filter_products(mems, is_reroll=False):
    ...
    def score(c):
        ...
        # 3. 오해된 메모리 (1차 추천용) 키워드 점수 - (새 로직에서는 작동하지 않음)
    ...
    if not st.session_state.messages:
        ...
        if purchase_context:
            ...
            ai_say(
                ...
                f"혹시 그 제품들을 구매하실 때 **'디자인'**이나 **'가격'**, **'성능/품질'** 중에서 어떤 점을 가장 중요하게 생각하셨나요?"
            )
...
def context_setting():
    st.title("💡 실험 준비: 초기 정보 수집")
    st.caption(f"AI 쇼핑 에이전트가 {st.session_state.nickname}님과의 대화를 시작할 맥락을 파악합니다.")
    
    st.markdown("---")
    st.markdown("#### 질문 1: 최근 3개월 동안 어떤 제품(카테고리)을 구매하셨나요? 두 가지 정도만 적어주세요.")
    st.caption("예: 옷, 신발, 시계, 화장품, 태블릿, 무선 키보드 등")
    
    purchase_list = st.text_input("최근 구매 품목 (2가지)", placeholder="예: 옷, 시계", key="purchase_list_input") 
    
    if st.button("AI와 대화 시작"):
        if purchase_list.strip():
            # 🚨 초기 메모리 생성 대신, 컨텍스트만 저장
